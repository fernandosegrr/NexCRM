  ...
  if (canal === "whatsapp") {
    const inicio = httpNode(
      ...
      [
        ...
        ["mediaBase64", "={{ $('Webhook').item.json.body.data.message.base64 ?? $('Webhook').item.json.body.data.message.imageMessage?.jpegThumbnail ?? $('Webhook').item.json.body.data.message.stickerMessage?.jpegThumbnail ?? $('Webhook').item.json.body.data.message.videoMessage?.jpegThumbnail ?? '' }}"],
